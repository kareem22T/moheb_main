        ...
        <div class="container">
            ...
            <div class="search">
                ...
                <div class="suggestion" v-if="searchArticles.length && search" style="font-size: 17px;position: absolute;top: 100%;display: flex;flex-direction: column;background: white;width: 100%;border-radius: 10px;margin-top: 10px;">
                    <a :href="`/term/${item.name}/${item.id}`"  v-for="item in searchArticles.slice(0, 5)" :key="item.id" style="font-size: 16px;border-bottom: 1px solid #80808052;padding: 5px 1rem;color: #1a3467;">@{{item.names[0].name}}<span>@{{ item.category.names[0].name }}</span></a>
